/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package br.ufsc.tests;

import br.ufsc.methods.MATSG_R;
import static br.ufsc.tests.TestTrajectoryCellSizeToRE.dir;
import static br.ufsc.tests.TestTrajectoryCellSizeToRE.extension;
import static br.ufsc.tests.TestTrajectoryCellSizeToRE.filename;
import java.io.IOException;
import java.text.ParseException;

/**
 *
 * @author vanes
 */
public class TestTrajectoryCellSize_Manual {

    public static String filename;
    public static String extension;
    public static String dir;

    public static void main(String[] args) throws IOException, ParseException, CloneNotSupportedException {

//        dir = "datasets\\Foursquare - user 6\\";
        dir = "datasets\\Experimento\\";
//        filename = "Foursquare - user 6 - v2";
        filename = "sanfrancisco_groundtruth_experimento[A2][formatted]";
        extension = ".csv";
        int z;
//        if(args.length > 101 ){            
//        z = Integer.parseInt(args[0]);
              z = 10;  
//        }

        MATSG_R method = new MATSG_R();

        //informando lista de att a ser forçados como categoricos, mesmo contendo números
//        String[] lstCategoricalsPreDefined = {"PRICE","rating".toUpperCase(),"label".toUpperCase(),"checkin_id".toUpperCase(),"venue_id".toUpperCase()};
        String[] lstCategoricalsPreDefined = {"gid", "taxi_id", "route", "semantic_stop_id", "semantic_move_id"};
        for (int i = 0; i < lstCategoricalsPreDefined.length; i++) {
            lstCategoricalsPreDefined[i] = lstCategoricalsPreDefined[i].toUpperCase();
        }
        //
        String patternDateIn = "yyyy-MM-dd";
//        String patternDateIn = "?"; // minutes data

        String SEPARATOR = ",";
        String[] valuesNulls = {"Unknown", "*-1", "*-999", "", "*"};

        float rc = 0.005f;
        float threshold_rv = 0.15f;
        String[] lstIgnoreColumns = {"label".toUpperCase(), "time".toUpperCase()};
        
//            method.notConsiderNulls();
            method.execute(dir, filename, extension, lstCategoricalsPreDefined, SEPARATOR, valuesNulls, lstIgnoreColumns, patternDateIn, rc, threshold_rv);
    }
}
